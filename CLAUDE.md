...
- Records the organization-wide star total in `metadata.org_star_total` /
  `metadata.org_repo_count` (see "Two different star totals" below)

The `repositories.py` module loads this JSON file and provides the `REPOSITORIES` list for other scripts, plus `load_org_metadata()` for the `metadata` block.
...
- **Two different star totals, three different repository sets**: the dashboard's
  *Total Stars* is `org_star_total` — the sum of `stargazerCount` over **every
  ...
  It is computed in `fetch_repositories.py`, which is the only script that
  already pages the whole organization, so it costs no extra API calls; it then
  travels through `repositories.json` → `calculate_stargazers_history.py` →
  `stars_history.json` so the page reads every star figure from one file.
  Private repositories are excluded (via `isPrivate`) to keep the number
  reproducible for a token with broader access.
  It is a **separate key** from `total_current_stars` on purpose:
